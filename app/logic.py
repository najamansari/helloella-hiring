...
async def get_or_create_transform_id(db: AsyncSession, list_1: List[str], list_2: List[str]):
    input_hash = hash_inputs(list_1, list_2)
    transformed = await get_transform(db, input_hash)

    if not transformed:
        transformed = apply_transform(list_1, list_2)
        await store_transform(db, transformed, input_hash)

    return input_hash


def apply_transform(list_1: List[str], list_2: List[str]):
    return [element for tup in zip(list_1, list_2) for element in tup]


def hash_inputs(list_1: List[str], list_2: List[str]):
    string = "".join(["".join(list_1), "".join(list_2)])
    return hashlib.sha1(string.encode()).hexdigest()


async def get_transform(db:AsyncSession, input_hash: str):
    stmt = (
...
async def store_transform(db:AsyncSession, transformed: List[str], input_hash: str):
